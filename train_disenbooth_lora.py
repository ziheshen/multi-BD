...
def forward_ctx_embeddings(input_image, text_input, qformer, proj_layer, ratio=None):
    def compute_ctx_embeddings(input_image, text_input):
        # blip_embeddings = self.blip(image=input_image, text=text_input)
        blip_embeddings = qformer.extract_features(
...
def extract_multimodal_features(input_image, text_input, qformer):
    multimodal_embeds = qformer.extract_features(
        {"image": input_image, "text_input": text_input}, mode="multimodal"
    ).multimodal_embeds
...
def main(args):
    ...
    qformer.to(accelerator.device)

    proj_layer = ProjLayer(
            in_dim=768, out_dim=1024, hidden_dim=3072, drop_p=0.1, eps=1e-12
        )
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            for model in models:
                ...
                elif isinstance(model, type(accelerator.unwrap_model(proj_layer))):
                    torch.save(model.state_dict() ,os.path.join(output_dir,"proj_layer.pt"))
    ...
    if args.train_text_encoder:
        unet, text_encoder, optimizer, train_dataloader, lr_scheduler, img_model, qformer, proj_layer, img_adapter = accelerator.prepare(
            unet, text_encoder, optimizer, train_dataloader, lr_scheduler, img_model, qformer, proj_layer, img_adapter
        )
    else:
        unet, optimizer, train_dataloader, lr_scheduler, img_model, qformer, proj_layer, img_adapter = accelerator.prepare(
            unet, optimizer, train_dataloader, lr_scheduler, img_model, qformer, proj_layer, img_adapter
        )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # Use Q-Former to get identity-preserving embeddings
                
                ctx_embeddings = forward_ctx_embeddings(pixel_values, ['mit_mascot'], qformer, proj_layer)
                
                encoder_hidden_states = text_encoder(
                    input_ids=batch['input_ids'],
                    ctx_embeddings=ctx_embeddings,
                    ctx_begin_pos=[2] * batch['input_ids'].shape[0],
                )[0]
        ...
        #         pipeline_args = {"prompt": args.validation_prompt, "negative_prompt": negative_prompt}
